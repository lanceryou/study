...
划分领域
> + 核心领域（推送领域）
> + 支撑领域 (用户筛选领域，运营统计领域，用户管理领域，群组活动管理领域，风控领域)
...
在梳理需求，输出业务模型后，我们就需要开始做系统设计与子系统的架构设计。系统设计的目的是减少噪音，聚焦业务与模型

系统设计可参考TOGAF:
开放组件体系结构框架（The Open Group Architecture Framework），这是一种高层设计方法，通常会按照维度
建模:
+ 业务架构
> 关键业务战略及其对业务功能和流程影响的表达, 一般来说也会指导数据架构，技术架构的设计
+ 应用架构
> 应用之间结构和交互的描述
+ 数据架构
> 描述组织的逻辑与物理数据资产及数据管理资源的结构
+ 技术架构
> 技术架构是用来描述支持业务、数据、应用服务部署的基础设施能力，
定义了企业的技术路线、技术标准、技术选择和技术组件等

在实际系统设计开发过程，我比较注重业务架构，应用架构(与逻辑架构类似)。在系统设计这层往往不关心具体子系统的架构设计，
系统设计关注的是整体的业务建模，和子系统之间交互联系，而架构设计则更进一步的需要关注业务需求，性能指标等方面，从而
作出合理的架构分层
...
![avatar](promotion-biz-architecture.png) 
应用架构


### 如何架构设计

架构设计我们可以根据4+1架构方法(The 4+1 View Model of Architecture)设计:
+ 用例架构
+ 逻辑架构
+ 处理架构
+ 物理架构
